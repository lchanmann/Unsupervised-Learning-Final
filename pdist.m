...
%PDIST Pairwise distance between observations. (identical to matla built in
%function, except this one displays a progress bar when the user specifies
...
%                       example @DISTFUN. (displays progress bar)
%
...
elseif ~strcmp(dist,'usr') && isfloat(X) % ~usr => ~complex
    ...
    h = waitbar(0,'Verifying distance function');
    try
    ...
    k = 1;
    for i = 1:n-1
        waitbar(k/(n*(n-1)/2),h,sprintf('Finding distances for sample %d of %d',i,n))
        try
